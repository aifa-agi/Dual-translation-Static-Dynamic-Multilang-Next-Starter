...
export function CookieBanner({
  ...
  const { consent, isLoaded, setConsent, saveConsent, acceptAll, rejectAll } = useCookieConsent();
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <div className="flex-shrink-0 mt-1">
                    <Image src={appConfig.logo} alt={appConfig.short_name} width={32} height={32} className="rounded" priority />
                  </div>
                  <h3
                ...
                <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <span>•</span>
                  <Link href={`mailto:${appConfig.mailSupport}`} className="text-blue-600 dark:text-blue-400 hover:underline">
          ...
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <Image src={appConfig.logo} alt={appConfig.short_name} width={40} height={40} className="rounded" priority />
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                    {t('cuid_cookie_settings_title')}
                  </h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {formatI18n(t('cuid_cookie_settings_subtitle'), { brand: appConfig.short_name})}
                  </p>
                </div>
              </div>
              <Button
            ...
            <div className="space-y-4 mb-8">
              {COOKIE_CATEGORIES.map(({ id, titleKey, descriptionKey, icon, color }) => {
                ...
                const Icon = ({ className }: { className?: string }) => {
                  switch (icon) {
                    case 'Shield':
                      return <svg className={className} /* svg content or import Icon from lucide-react */ role="img" aria-label={t(titleKey)} />;
                    case 'BarChart3':
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {id !== 'essential' && (
                          <button
                            onClick={() => setConsent({ ...consent, [id]: !enabled })}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="flex gap-3">
                <Button variant="outline" size="sm" onClick={rejectAll} className="font-medium">
                  {t('cuid_cookie_banner_reject_all')}
                </Button>
                <Button
                  size="sm"
